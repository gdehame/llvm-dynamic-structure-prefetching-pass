...
void *HashLookup(unsigned int key, Hash hash)
{
  ...
  pthread_create(&thread_id, NULL, ht, NULL);
  ent_ht = hash->array[(hash->mapfunc)(key)];
  key_ht = key;
  pthread_barrier_wait(&barrier);
  int j;
  HashEntry ent;
  ...
  for (ent = hash->array[j];       /* 17% miss in hash->array[j] */ /* adt_pf can't detect :( */
       ent &&                      /* 47% miss in ent->key */       /* adt_pf can detect :) */
           ent->key!=key; 
       ent=ent->next); // 46% L1 mr             /* 8% miss in ent->next */       /* adt_pf can detect :) */
